...
Categorías que reconozco automáticamente: comida, transporte, vivienda, ocio, salud, ropa, educación, trabajo, otros.`;
}
...
export function buildSummary(phone) {
  ...
  if (transactions.length === 0) {
    return '📊 Aún no tienes movimientos registrados este mes. Escríbeme algo como "gasté 20 en comida" para empezar.';
  }
...
export function buildHistory(phone) {
  ...
  for (const t of transactions) {
    ...
    const date = new Date(t.created_at + 'Z').toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit' });
